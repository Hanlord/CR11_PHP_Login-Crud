...
if (isset($_POST["submit"])) {
    ...
    if ($pictureArray->error === 0) {
        ...
        $sql = "UPDATE users SET first_name = '$f_name', last_name = '$l_name', email = '$email', address = '$address', picture = '$pictureArray->fileName' WHERE id = {$id}";
    } else {
        $sql = "UPDATE users SET first_name = '$f_name', last_name = '$l_name', email = '$email', address = '$address' WHERE id = {$id}";
    }
    if (mysqli_query($connect, $sql) === true) {
        $class = "alert alert-success";
        $message = "The record was successfully updated";
        $uploadError = ($pictureArray->error != 0) ? $pictureArray->ErrorMessage : '';
        header("refresh:100;url=update.php?id={$id}");
    } else {
